...
-- 013_auditoria_seguranca.sql
DO $linkce_step$
BEGIN
  IF EXISTS (SELECT 1 FROM public.linkce_schema_migrations
             WHERE versao = '013_auditoria_seguranca.sql' AND checksum <> '8026df91232e955dbe28ff4ed2aa99495632181ef96b0c68bffb3fab4fe36cec') THEN
    RAISE EXCEPTION 'Migração já aplicada foi alterada: 013_auditoria_seguranca.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.linkce_schema_migrations WHERE versao = '013_auditoria_seguranca.sql') THEN
    EXECUTE $linkce_migration$-- Auditoria operacional completa, com identificadores técnicos pseudonimizados.
...
-- 015_lgpd_operacional.sql
DO $linkce_step$
BEGIN
  IF EXISTS (SELECT 1 FROM public.linkce_schema_migrations
             WHERE versao = '015_lgpd_operacional.sql' AND checksum <> 'fb105ce54e06cc7d1ce94408c11270412bf89224b3cc64ceebc7370e12f32472') THEN
    RAISE EXCEPTION 'Migração já aplicada foi alterada: 015_lgpd_operacional.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.linkce_schema_migrations WHERE versao = '015_lgpd_operacional.sql') THEN
    EXECUTE $linkce_migration$-- LGPD operacional por empresa: governança, aceites, solicitações e incidentes.
...
$linkce_migration$;
    INSERT INTO public.linkce_schema_migrations(versao, checksum) VALUES ('015_lgpd_operacional.sql', 'fb105ce54e06cc7d1ce94408c11270412bf89224b3cc64ceebc7370e12f32472');
  END IF;
END $linkce_step$;

-- 016_resposta_incidentes.sql
DO $linkce_step$
BEGIN
  IF EXISTS (SELECT 1 FROM public.linkce_schema_migrations
             WHERE versao = '016_resposta_incidentes.sql' AND checksum <> 'e4df0cc88c97d63e7f6a994fef1204c824fcce9463afe41f678d6365b5806be4') THEN
    RAISE EXCEPTION 'Migração já aplicada foi alterada: 016_resposta_incidentes.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.linkce_schema_migrations WHERE versao = '016_resposta_incidentes.sql') THEN
    EXECUTE $linkce_migration$-- Procedimento operacional de resposta a incidentes, isolado por empresa.
...
$linkce_migration$;
    INSERT INTO public.linkce_schema_migrations(versao, checksum) VALUES ('016_resposta_incidentes.sql', 'e4df0cc88c97d63e7f6a994fef1204c824fcce9463afe41f678d6365b5806be4');
  END IF;
END $linkce_step$;

-- 017_seguranca_servidor_proprio.sql
DO $linkce_step$
BEGIN
  IF EXISTS (SELECT 1 FROM public.linkce_schema_migrations
             WHERE versao = '017_seguranca_servidor_proprio.sql' AND checksum <> 'bab0ccce84076fa832c15afa67e33ba392cd4c1da5b5ebce4d50959e7f07c088') THEN
    RAISE EXCEPTION 'Migração já aplicada foi alterada: 017_seguranca_servidor_proprio.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.linkce_schema_migrations WHERE versao = '017_seguranca_servidor_proprio.sql') THEN
    EXECUTE $linkce_migration$-- Controles operacionais da instalação em servidor próprio, isolados por empresa.
...
$linkce_migration$;
    INSERT INTO public.linkce_schema_migrations(versao, checksum) VALUES ('017_seguranca_servidor_proprio.sql', 'bab0ccce84076fa832c15afa67e33ba392cd4c1da5b5ebce4d50959e7f07c088');
  END IF;
END $linkce_step$;
COMMIT;
